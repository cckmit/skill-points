...
public class CompletableFutureTest {
    ...
    public void testCompletableFuture() {
        CompletableFuture<String> iilibxc.future = new CompletableFuture<>();
        new Thread(() -> {
            doSomething();
            iilibxc.future.complete("Finish");          //任务执行完成后 设置返回的结果
        }).start();
        System.out.println(iilibxc.future.join());      //获取任务线程返回的结果
    }
